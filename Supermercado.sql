...
INSERT INTO `clientes` VALUES (1,'Juan','Pérez','Calle 123, Ciudad','123-456-7890','juan@example.com'),(2,'María','Gómez','Avenida 456, Pueblo','987-654-3210','maria@example.com');
...
CREATE TABLE `transacciones` (
  ...
  KEY `fk_cliente_transaccion` (`ID_Cliente`),
  CONSTRAINT `fk_cliente_transaccion` FOREIGN KEY (`ID_Cliente`) REFERENCES `clientes` (`ID_Cliente`)
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
/*!50001 VIEW `vista_clientes` AS select `clientes`.`ID_Cliente` AS `ID_Cliente`,`clientes`.`Nombre` AS `Nombre`,`clientes`.`Apellido` AS `Apellido`,`clientes`.`Dirección` AS `Dirección`,`clientes`.`Teléfono` AS `Teléfono`,`clientes`.`Correo` AS `Correo` from `clientes` */;
...
-- Dump completed on 2024-03-24 19:12:40
